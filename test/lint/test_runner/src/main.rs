...
fn get_linter_list() -> Vec<&'static Linter> {
    vec![
        ...
        &Linter {
            description: "Check that no symbol from sixbit-build-config.h is used without the header being included",
...
fn parse_lint_args(args: &[String]) -> Vec<&'static Linter> {
    ...
    for arg in args {
        ...
        if arg.starts_with("--lint=") {
            ...
            let try_find_linter = linter_list.iter().find(|linter| linter.name == lint_arg_value);
...
fn lint_py_lint() -> LintResult {
    ...
    let checks = format!(
        "--select={}",
        [
            "B006", "B008", "E101", "E401", "E402", "E701", "E702", "E703", "E711", "E714",
            "E721", "E722", "E742", "E743", "F401", "F402", "F403", "F404", "F405", "F406",
            "F407", "F541", "F601", "F602", "F621", "F631", "F632", "F811", "F821", "F822",
            "F823", "F841", "PLE", "W191", "W291", "W292", "W293", "W605",
        ]
        .join(",")
    );
    ...
    match cmd.status() {
        ...
        Err(e) if e.kind() == ErrorKind::NotFound => {
            println!("`{}` was not found in $PATH, skipping those checks.", bin_name);
            Ok(())
        }
...
fn lint_tabs_whitespace() -> LintResult {
    let tabs = git()
        .args(["grep", "-I", "--line-number", "--perl-regexp", r"^\t", "--"])
        .args(["*.cpp", "*.h", "*.md", "*.py", "*.sh"])
        .args(get_pathspecs_exclude_whitespace())
        .status()
        .expect("command error")
...
fn lint_includes_build_config() -> LintResult {
    let config_path = "./cmake/sixbit-build-config.h.in";
    ...
    if missing {
        return Err(
            format!(
                r#"
One or more files use a symbol declared in the sixbit-build-config.h header. However, they are not
...
even though sixbit-build-config.h indicates that a faster feature is available and should be used.
...
#include <sixbit-build-config.h> // IWYU pragma: keep
    ...
    if redundant {
        return Err(
            r#"
None of the files use a symbol declared in the sixbit-build-config.h header. However, they are including
...
{}
        ...
        Err(e) => Err(format!("Error running mlc: {}", e)),
    }
}
...
fn main() -> ExitCode {
    ...
    for linter in linters_to_run {
        ...
        if let Err(err) = (linter.lint_fn)() {
            println!(
                "^^^\n{err}\n^---- Failure generated from lint check '{}' ({})!\n\n",
