...
while getopts "?hr" opt; do
  case $opt in
    '?' | h)
      ...
      echo ""
      echo "            git fetch <upstream-subtree-remote>"
...
find_latest_squash()
{
    ...
    main=
    sub=
    git log --grep="^git-subtree-dir: $dir/*\$"         --pretty=format:'START %H%n%s%n%n%b%nEND%n' "$commit" |
    while read a b _; do
        case "$a" in
            START) sq="$b" ;;
...
tree_actual=$(git ls-tree -d "$COMMIT" "$DIR")
