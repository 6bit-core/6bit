...
TMPDIR_PREFIX = "sixbit_func_test_"
...
class Binaries:
    """Helper class to provide information about sixbit binaries
    ...
    def node_argv(self, **kwargs):
        "Return argv array that should be used to invoke sixbitd"
        return self._argv("node", self.paths.sixbitd, **kwargs)

    def rpc_argv(self):
        "Return argv array that should be used to invoke sixbit-cli"
        # Add -nonamed because "sixbit rpc" enables -named by default, but sixbit-cli doesn't
        return self._argv("rpc", self.paths.sixbitcli) + ["-nonamed"]

    def tx_argv(self):
        "Return argv array that should be used to invoke sixbit-tx"
        return self._argv("tx", self.paths.sixbittx)

    def util_argv(self):
        "Return argv array that should be used to invoke sixbit-util"
        return self._argv("util", self.paths.sixbitutil)

    def wallet_argv(self):
        "Return argv array that should be used to invoke sixbit-wallet"
        return self._argv("wallet", self.paths.sixbitwallet)

    def chainstate_argv(self):
        "Return argv array that should be used to invoke sixbit-chainstate"
        return self._argv("chainstate", self.paths.sixbitchainstate)

    def _argv(self, command, bin_path, need_ipc=False):
        """Return argv array that should be used to invoke the command. It
        either uses the sixbit wrapper executable (if sixbit_CMD is set or
        need_ipc is True), or the direct binary path (sixbitd, etc). When
        bin_dir is set (by tests calling binaries from previous releases) it
        always uses the direct path."""
        if self.bin_dir is not None:
            return [os.path.join(self.bin_dir, os.path.basename(bin_path))]
        elif self.paths.sixbit_cmd is not None or need_ipc:
            # If the current test needs IPC functionality, use the sixbit
            # wrapper binary and append -m so it calls multiprocess binaries.
            sixbit_cmd = self.paths.sixbit_cmd or [self.paths.sixbit_bin]
            return sixbit_cmd + (["-m"] if need_ipc else []) + [command]
        else:
            return [bin_path]


class SixBitTestMetaClass(type):
    """Metaclass for SixBitTestFramework.

    Ensures that any attempt to register a subclass of `SixBitTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'SixBitTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("SixBitTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("SixBitTestFramework subclasses may not override "
                                "'__init__' or 'main'")
...
class SixBitTestFramework(metaclass=SixBitTestMetaClass):
    """Base class for a sixbit test script.

    Individual sixbit test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self, test_file):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave sixbitds and test.* datadir on exit or error")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use sixbit-cli instead of RPC for all commands")
    ...
    def get_binary_paths(self):
        ...
        binaries = {
            "sixbit": "sixbit_BIN",
            "sixbitd": "sixbitD",
            "sixbit-cli": "sixbitCLI",
            "sixbit-util": "sixbitUTIL",
            "sixbit-tx": "sixbitTX",
            "sixbit-chainstate": "sixbitCHAINSTATE",
            "sixbit-wallet": "sixbitWALLET",
        }
        # Set paths to sixbit core binaries allowing overrides with environment
        # variables.
        ...
        # sixbit_CMD environment variable can be specified to invoke sixbit
        # wrapper binary instead of other executables.
        paths.sixbit_cmd = shlex.split(os.getenv("sixbit_CMD", "")) or None
        return paths
    ...
    def shutdown(self):
        ...
        rpc_logger = logging.getLogger("sixbitRPC")
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a sixbitd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple sixbitd test nodes"""
        for node in self.nodes:
    ...
    def create_outpoints(self, node, *, outputs):
        """Send funds to a given list of `{address: amount}` targets using the sixbitd
    ...
    def _start_logging(self):
        ...
        # Format logs the same as sixbitd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in sixbit.conf
    ...
    def skip_if_no_sixbitd_tracepoints(self):
        """Skip the running test if sixbitd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("sixbitd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_sixbitd_zmq(self):
        """Skip the running test if sixbitd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("sixbitd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if sixbit-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("sixbit-wallet has not been compiled")

    def skip_if_no_sixbit_tx(self):
        """Skip the running test if sixbit-tx has not been compiled."""
        if not self.is_sixbit_tx_compiled():
            raise SkipTest("sixbit-tx has not been compiled")

    def skip_if_no_sixbit_util(self):
        """Skip the running test if sixbit-util has not been compiled."""
        if not self.is_sixbit_util_compiled():
            raise SkipTest("sixbit-util has not been compiled")

    def skip_if_no_sixbit_chainstate(self):
        """Skip the running test if sixbit-chainstate has not been compiled."""
        if not self.is_sixbit_chainstate_compiled():
            raise SkipTest("sixbit-chainstate has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if sixbit-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("sixbit-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether sixbit-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether sixbit-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_sixbit_tx_compiled(self):
        """Checks whether sixbit-tx was compiled."""
        return self.config["components"].getboolean("BUILD_sixbit_TX")

    def is_sixbit_util_compiled(self):
        """Checks whether sixbit-util was compiled."""
        return self.config["components"].getboolean("ENABLE_sixbit_UTIL")

    def is_sixbit_chainstate_compiled(self):
        """Checks whether sixbit-chainstate was compiled."""
        return self.config["components"].getboolean("ENABLE_sixbit_CHAINSTATE")
