...
class ToolSixBitTest(SixBitTestFramework):
    def set_test_params(self):
        self.setup_clean_chain = True
        self.num_nodes = 1

    def skip_test_if_missing_module(self):
        # Skip test on windows because currently when `sixbit node -version` is
        # run on windows, python doesn't capture output from the child
        # `sixbitd` and `sixbit-node` process started with _wexecvp, and
        # stdout/stderr are always empty. See
        # https://github.com/sixbit/sixbit/pull/33229#issuecomment-3265524908
    ...
    def set_cmd_args(self, node, args):
        """Set up node so it will be started through sixbit wrapper command with specified arguments."""
        node.args = [self.binary_paths.sixbit_bin] + args + ["node"] + self.node_options[node.index]
    ...
    def run_test(self):
        ...
        self.log.info("Ensure sixbit node command invokes sixbitd by default")
        self.test_args([], [], expect_exe="sixbitd")

        self.log.info("Ensure sixbit -M invokes sixbitd")
        self.test_args(["-M"], [], expect_exe="sixbitd")

        self.log.info("Ensure sixbit -M does not accept -ipcbind")
        ...
        if self.is_ipc_compiled():
            self.log.info("Ensure sixbit -m invokes sixbit-node")
            self.test_args(["-m"], [], expect_exe="sixbit-node")

            self.log.info("Ensure sixbit -m does accept -ipcbind")
            self.test_args(["-m"], ["-ipcbind=unix"], expect_exe="sixbit-node")

            self.log.info("Ensure sixbit accepts -ipcbind by default")
            self.test_args([], ["-ipcbind=unix"], expect_exe="sixbit-node")

            self.log.info("Ensure sixbit recognizes -ipcbind in config file")
            append_config(node.datadir_path, ["ipcbind=unix"])
            self.test_args([], [], expect_exe="sixbit-node")
