...
class TransactionTimeRescanTest(SixBitTestFramework):
    ...
    def run_test(self):
        ...
        minernode = self.nodes[0]  # node used to mine coins and create transactions
        ...
        t1 = cur_time + ten_days
        t2 = cur_time + 2 * ten_days
        t3 = cur_time + 3 * ten_days
        t4 = cur_time + 4 * ten_days
        ...
        # generate coins to create transactions and check blockcount
        ...
        # synchronize nodes and time
        self.sync_all()
        set_node_times(self.nodes, t1)

        # send 10 coins to user's first watch-only address
        self.log.info('Send 10 coins to user')
        ...
        # synchronize nodes and time
        self.sync_all()
        set_node_times(self.nodes, t2)

        # send 5 coins to our second watch-only address
        self.log.info('Send 5 coins to user')
        ...
        # synchronize nodes and time
        self.sync_all()
        set_node_times(self.nodes, t3)

        # send 1 coin to our third watch-only address
        self.log.info('Send 1 coin to user')
        ...
        for tx in wo_wallet.listtransactions():
            if tx['address'] == wo1:
                assert_equal(tx['blocktime'], t1)
                assert_equal(tx['time'], t1)
            elif tx['address'] == wo2:
                assert_equal(tx['blocktime'], t2)
                assert_equal(tx['time'], t2)
            elif tx['address'] == wo3:
                assert_equal(tx['blocktime'], t3)
                assert_equal(tx['time'], t3)
