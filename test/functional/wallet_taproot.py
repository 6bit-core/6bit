...
from test_framework.test_framework import SixBitTestFramework
...
def compute_raw_taproot_address(pubkey):
    """Compute a raw regtest taproot address from an output pubkey."""
    return encode_segwit_address("bcrt", 1, pubkey)


class WalletTaprootTest(SixBitTestFramework):
    ...
    def next_wallet_id(self):
        self._wallet_counter += 1
        return f"{self._wallet_counter:04d}"
    ...
    def make_addr(treefn, keys, i):
        ...
        raise AssertionError("Unexpected tree type returned by treefn")

    def do_test_addr(self, comment, pattern, privmap, treefn, keys):
        self.log.info("Testing %s address derivation" % comment)

        wallet_id = self.next_wallet_id()
        self.nodes[0].createwallet(wallet_name=f"privs_tr_enabled_{wallet_id}", blank=True)
        self.nodes[0].createwallet(wallet_name=f"pubs_tr_enabled_{wallet_id}", blank=True, disable_private_keys=True)
        self.nodes[0].createwallet(wallet_name=f"addr_gen_{wallet_id}", disable_private_keys=True, blank=True)
        privs_tr_enabled = self.nodes[0].get_wallet_rpc(f"privs_tr_enabled_{wallet_id}")
        pubs_tr_enabled = self.nodes[0].get_wallet_rpc(f"pubs_tr_enabled_{wallet_id}")
        addr_gen = self.nodes[0].get_wallet_rpc(f"addr_gen_{wallet_id}")
    ...
    def do_test_sendtoaddress(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        wallet_id = self.next_wallet_id()
        self.nodes[0].createwallet(wallet_name=f"rpc_online_{wallet_id}", blank=True)
        rpc_online = self.nodes[0].get_wallet_rpc(f"rpc_online_{wallet_id}")
        ...
        for i in range(4):
            ...
            to_amnt = self.random.randrange(1000000, boring_balance)
            ...
            ret_amnt = self.random.randrange(100000, test_balance)
    ...
    def do_test_psbt(self, comment, pattern, privmap, treefn, keys_pay, keys_change):
        ...
        wallet_id = self.next_wallet_id()
        self.nodes[0].createwallet(wallet_name=f"psbt_online_{wallet_id}", disable_private_keys=True, blank=True)
        self.nodes[1].createwallet(wallet_name=f"psbt_offline_{wallet_id}", blank=True)
        self.nodes[1].createwallet(wallet_name=f"key_only_wallet_{wallet_id}", blank=True)
        psbt_online = self.nodes[0].get_wallet_rpc(f"psbt_online_{wallet_id}")
        psbt_offline = self.nodes[1].get_wallet_rpc(f"psbt_offline_{wallet_id}")
        key_only_wallet = self.nodes[1].get_wallet_rpc(f"key_only_wallet_{wallet_id}")
        ...
        assert result[0]['success']
        for key_item in keys_pay + keys_change:
            result = key_only_wallet.importdescriptors([{"desc": descsum_create(f"wpkh({key_item['xprv']}/*)"), "timestamp": "now"}])
            assert result[0]["success"]
    ...
    def do_test(self, comment, pattern, privmap, treefn):
        nkeys = len(privmap)
        keys = self.random.sample(KEYS, nkeys * 4)
    ...
    def run_test(self):
        ...
        rnd_pos = self.random.randrange(MAX_PUBKEYS_PER_MULTI_A)
        self.do_test(
