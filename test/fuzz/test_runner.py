...
def main():
    ...
    fuzz_bin = (
        os.getenv('SIXBITFUZZ')
        or os.getenv('SIXBITFUZZ')
        or os.path.join(config['environment']['BUILDDIR'], 'bin', 'fuzz')
    )
    ...
    if not args.generate:
        ...
        if test_list_missing_corpus:
            ...
            logging.info('Please consider adding a fuzz corpus at https://github.com/sixbit-core/sixbit/qa-assets')
...
def generate_corpus(*, fuzz_pool, src_dir, fuzz_bin, corpus_dir, targets):
    ...
    for target, t_env in targets:
        ...
        use_value_profile = int(rng.random() < 0.3)
        command = [
            fuzz_bin,
            '-rss_limit_mb=8000',
            '-max_total_time=6000',
...
def run_once(*, fuzz_pool, corpus, test_list, src_dir, fuzz_bin, using_libfuzzer, use_valgrind, empty_min_time):
    ...
    for t in test_list:
        ...
        if using_libfuzzer:
            if empty_min_time and empty_dir:
                args += [f'-max_total_time={empty_min_time}', corpus_path]
            else:
                args += [
                    '-runs=1',
                    corpus_path,
    ...
    for future in as_completed(jobs):
        ...
        try:
            ...
            logging.info(f'Failure generated from target with exit code {e.returncode}: {result.args}')
            sys.exit(1)
        if using_libfuzzer:
