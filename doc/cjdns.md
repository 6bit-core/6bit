# CJDNS support in Sixbit Core

It is possible to run Sixbit Core over CJDNS, an encrypted IPv6 network that
...
done outside of Sixbit Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the sixbit network and individual nodes.
...
before it will work with your Sixbit Core node. This is described in steps
...
## Run Sixbit Core with CJDNS

Once you are connected to the CJDNS network, the following Sixbit Core
...
When enabled, this option tells Sixbit Core that it is running in an
...
IPv6 local network. This helps Sixbit Core perform better address management:
...
CJDNS support was added to Sixbit Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `sixbit-cli -addrinfo` to see the
...
the networks has issues. There are a number of ways to configure this.

## CJDNS-related information in Sixbit Core

There are several ways to see your CJDNS address in Sixbit Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `sixbit-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `sixbit-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `sixbit-cli help getnodeaddresses` for details.
