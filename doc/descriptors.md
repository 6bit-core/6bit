# Support for Output Descriptors in Sixbit Core

Since Sixbit Core v0.17, there is support for Output Descriptors. This is a
...
- Followed by the actual key, which is either:
  - Hex encoded public keys (either 66 characters starting with `02` or `03` for a compressed pubkey, or 130 characters starting with `04` for an uncompressed pubkey).
    ...
    - `xpub` encoded extended public key or `xprv` encoded extended private key.
...
- Segwit addresses (bech32 and bech32m, of the form `bc1...` for mainnet or `tb1...` for testnet.
...
on sixbit's OP_CHECKMULTISIG opcode. To support these, we introduce the
...
transactions related to the multisig. Lastly, it is not recommended to use anything other than a Sixbit Core descriptor wallet to serve as your
...
the entire path from the master down. By
...
existing Sixbit Core wallets, a convenience function `combo` is
...
All RPCs in Sixbit Core will include the checksum in their output. Only
