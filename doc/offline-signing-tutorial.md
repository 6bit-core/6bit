...
> ?? SECURITY WARNING
>
> The offline machine must NEVER be connected to any network
> (internet, Wi-Fi, Bluetooth, USB networking, etc.).
> Compromising this isolation defeats the purpose of this setup.


This tutorial will describe how to use two instances of Sixbit Core, one online and one offline, to greatly increase security by not having private keys reside on a networked device.
...
This workflow uses [Partially Signed sixbit Transactions](https://github.com/6bit-core/6bit/blob/master/doc/psbt.md) (PSBTs) to transfer the transaction to and from the offline wallet for signing using the private keys.
...
## Overview
In this tutorial we have two hosts, both running sixbit v1.0.0
...
```sh
[offline]$ ./build/bin/sixbit-cli -signet -named createwallet \
...
```sh
[offline]$ ./build/bin/sixbit-cli -signet -rpcwallet="offline_wallet" listdescriptors \
             | jq -r '.descriptors' \
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -named createwallet \
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" importdescriptors "$(cat /path/to/descriptors.json)"
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" getnewaddress

tb1qtu5qgc6ddhmqm5yqjvhg83qgk2t4ewajg0h6yh
```

2. Visit a faucet like https://faucet.6bit.dev and enter your address from the previous command to receive a small amount of signet coins to this address.
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" listunspent
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" send \
...
```sh
[offline]$ ./build/bin/sixbit-cli -signet decodepsbt $(cat /path/to/funded_psbt.txt)

{
    ...
}

[offline]$ ./build/bin/sixbit-cli -signet analyzepsbt $(cat /path/to/funded_psbt.txt)
...
```sh
[offline]$ ./build/bin/sixbit-cli -signet -rpcwallet="offline_wallet" walletpassphrase "** enter passphrase **" 60
```
...
2. Process, sign and finalize the PSBT on the `offline_wallet` using the `walletprocesspsbt` command, saving the output to a file `final_psbt.txt`.
 ...
 ```sh
[offline]$ ./build/bin/sixbit-cli -signet -rpcwallet="offline_wallet" walletprocesspsbt \
...
```sh
[online]$ ./build/bin/sixbit-cli -signet sendrawtransaction $(cat /path/to/final_psbt.txt)
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" getbalances
...
```sh
[online]$ ./build/bin/sixbit-cli -signet -rpcwallet="watch_only_wallet" listtransactions
...
After signing, securely delete:

- funded_psbt.txt
- descriptors.json (if no longer needed)

from any intermediate devices.


[offline wallet] --(descriptors)--> [online watch-only]
[online] --(PSBT)--> [offline]
[offline] --(signed tx)--> [online]
