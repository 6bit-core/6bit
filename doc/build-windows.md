...
Below are some notes on how to build Sixbit Core for Windows.

The options known to work for building Sixbit Core on Windows are:
...
Note that for WSL the Sixbit Core source path MUST be somewhere in the default mount file system, for
example /usr/src/sixbit, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\sixbit`, for example:
```shell
cmake --install build --prefix /mnt/c/workspace/sixbit
```
...
```shell
cmake --install build --prefix /mnt/c/workspace/sixbit --strip
```
