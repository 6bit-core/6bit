...
Since version 0.21, Sixbit Core no longer has a default wallet.
...
The following command, for example, creates a descriptor wallet. More information about this command may be found by running `sixbit-cli help createwallet`.

```
$ sixbit-cli createwallet "wallet-01"
```

`sixbit-cli` is the command-line interface for interacting with the node.
...
| -----------------|:------------------------------------------------------------|
| Linux            | `/home/<user>/.sixbit/wallets`                             |
| Windows          | `C:\Users\<user>\AppData\Local\sixbit\wallets`             |
| macOS            | `/Users/<user>/Library/Application Support/sixbit/wallets` |
...
Wallet encryption does not protect against all attack vectors. An attacker can, for example, obtain the password by installing a keylogger on the user's machine.

After encrypting the wallet or changing the passphrase, a new backup needs to be created immediately. The reason is that the keypool is flushed and a new HD seed is generated after encryption. Any sixbits received by the new seed cannot be recovered from the previous backups.

The wallet's private key may be encrypted with the following command:

```
$ sixbit-cli -rpcwallet="wallet-01" encryptwallet "passphrase"
```

Once encrypted, the passphrase can be changed with the `walletpassphrasechange` command.

```
$ sixbit-cli -rpcwallet="wallet-01" walletpassphrasechange "oldpassphrase" "newpassphrase"
```

The argument passed to `-rpcwallet` specifies the wallet to be encrypted.
...
```
$ sixbit-cli -named createwallet wallet_name="wallet-01" passphrase="passphrase"
```
...
```
$ sixbit-cli -rpcwallet="wallet-01" sendtoaddress "sixbit1qexamplexxxxxxxxxxxxxxxxxxxxxx" 0.01
error code: -13
error message:
...
```
$ sixbit-cli -rpcwallet="wallet-01" walletpassphrase "passphrase" 120
```
...
```
$ sixbit-cli -rpcwallet="wallet-01" backupwallet /home/node01/Backups/backup-01.dat
```
...
If both the wallet and all backups are lost for any reason, the coins in this wallet will become permanently inaccessible.

### 1.5 Backup Frequency

The original Sixbit Core wallet was a collection of unrelated private keys. If a non-HD wallet had received funds to an address and then was restored from a backup made before the address was generated, then any funds sent to that address would have been lost because there was no deterministic mechanism to derive the address again.

Sixbit Core [version 0.13](https://github.com/6bit-core/6bit/blob/master/doc/release-notes/release-notes-1.0.0.md) introduced HD wallets with deterministic key derivation. With HD wallets, users no longer lose funds when restoring old backups because all addresses are derived from the HD wallet seed.
...
```
$ sixbit-cli restorewallet "restored-wallet" /home/node01/Backups/backup-01.dat
```

After that, `getwalletinfo` can be used to check if the wallet has been fully restored.

```
$ sixbit-cli -rpcwallet="restored-wallet" getwalletinfo
```
...
Understanding wallet security is crucial for safely storing your sixbit. A key aspect is the wallet passphrase, used for encryption. Let's explore its nuances, role, encryption process, and limitations.
