...
* Update sixbit.conf and commit changes if they exist..
...
Checkout the Sixbit Core version you'd like to build:
...
Then open a Pull Request to the [guix.sigs repository](https://github.com/sixbit-core/guix.sigs).
...
In the `guix-build-${VERSION}/output/x86_64-apple-darwin` and `guix-build-${VERSION}/output/arm64-apple-darwin` directories:
    ...
    tar xf sixbit-${VERSION}-${ARCH}-apple-darwin-codesigning.tar.gz
...
In the `guix-build-${VERSION}/output/x86_64-w64-mingw32` directory:
    ...
    tar xf sixbit-${VERSION}-win64-codesigning.tar.gz
...
However if this is done, once the release has been tagged in the sixbit-detached-sigs repo, the `guix-codesign` step must be performed again in order for the guix attestation to be valid when compared against the attestations of non-codesigner builds. The directories created by `guix-codesign` will need to be cleared prior to running `guix-codesign` again.

### Windows and macOS codesigners only: Commit the detached codesign payloads

```sh
pushd ./sixbit-detached-sigs
...
- Detached signatures will then be committed to the [sixbit-detached-sigs](https://github.com/sixbit-core/sixbit-detached-sigs) repository, which can be combined with the unsigned apps to create signed binaries.
...
- Upload to the sixbit server:
    1. The contents of each `./sixbit/guix-build-${VERSION}/output/${HOST}/` directory.
...
- After uploading release candidate binaries, notify the sixbit-core-dev mailing list and
  6bit.dev group that a release candidate is available for testing. Include a link to the release
  notes draft.
...
- Optionally help seed this torrent. To get the `magnet:` URI use:
  ...
  people without access to `sixbit` to download the binary distribution.
...
- Update the sixbit website

  - blog post

  - maintained versions [table](https://github.com/sixbit-core/commits/master/_includes/posts/maintenance-table.md)

  - RPC documentation update

      - See https://github.com/sixbit-core/sixbit/blob/master/contrib/doc-gen/


- Update repositories

  - Delete post-EOL [release branches](https://github.com/sixbit-core/sixbit/branches/all) and create a tag `v${branch_name}-final`.

  - Delete ["Needs backport" labels](https://github.com/sixbit-core/sixbit/labels?q=backport) for non-existing branches.

  - Update packaging repo

  - Create a [new GitHub release](https://github.com/sixbit-core/sixbit/releases/new) with a link to the archived release notes

- Announce the release:

  - 6bit.dev and sixbit-core-dev mailing list

  - Sixbit Core announcements list https://sixbit/en/list/announcements/join/

  - Sixbit Core Twitter https://twitter.com/6bit.dev
