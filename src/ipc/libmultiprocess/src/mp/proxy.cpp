...
Connection::~Connection()
{
    ...
    // object, which can run arbitrary blocking sixbit code so they have to run
    ...
    // functions run, so client objects are fully disconnected before sixbit
    // code in the destructors are run. This way if the sixbit code tries to
...
void EventLoop::addAsyncCleanup(std::function<void()> fn)
{
    ...
    //
    // In sixbit core, running these callbacks in the right order is
