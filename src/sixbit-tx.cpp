...
static void SetupSixBitTxArgs(ArgsManager &argsman)
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = CLIENT_NAME " sixbit-tx utility version " + FormatFullVersion() + "\n";

        if (gArgs.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The sixbit-tx tool is used for creating and modifying sixbit transactions.\n\n"
                "sixbit-tx can be used with \"<hex-tx> [commands]\" to update a hex-encoded sixbit transaction, or with \"-create [commands]\" to create a hex-encoded sixbit transaction.\n"
                "\n"
                "Usage: sixbit-tx [options] <hex-tx> [commands]\n"
                "or:    sixbit-tx [options] -create [commands]\n"
                "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded sixbit transaction
