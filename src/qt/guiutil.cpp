// Copyright (c) 2011-present The Sixbit Core developers
// Distributed under the MIT software license, see the accompanying
// file COPYING or https://opensource.org/license/mit/.

#include <qt/guiutil.h>

#include <qt/sixbitaddressvalidator.h>
...
static std::string DummyAddress(const CChainParams &params)
{
    ...
    case ChainType::MAIN:
        addr = "6b1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqxxxxxx";
        break;
    case ChainType::SIGNET:
    case ChainType::TESTNET:
    case ChainType::TESTNET4:
        addr = "t6b1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqxxxxxx";
        break;
    case ChainType::REGTEST:
        addr = "r6b1p35yvjel7srp783ztf8v6jdra7dhfzk5jaun8xz2qp6ws7z80n4tqxxxxxx";
        break;
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a sixbit address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new SixbitAddressEntryValidator(parent));
    widget->setCheckValidator(new SixbitAddressCheckValidator(parent));
}
...
bool parsesixbitURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or not a sixbit: URI
    if(!uri.isValid() || uri.scheme() != QString("sixbit"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!sixbitUnits::parse(sixbitUnit::SIXBIT, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parsesixbitURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsesixbitURI(uriInstance, out);
}

QString formatsixbitURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("sixbit:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(sixbitUnits::format(sixbitUnit::SIXBIT, info.amount, false, sixbitUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openSixbitConf()
{
    ...
    /* Open sixbit.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "sixbit.lnk";
    if (chain == ChainType::TESTNET) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "sixbit (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("sixbit (%s).lnk", ChainTypeToString(chain)));
}

bool GetStartOnSystemStartup()
{
    // check for sixbit*.lnk
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::MAIN)
        return GetAutostartDir() / "sixbit.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("sixbit-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a sixbit.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::MAIN)
            optionFile << "Name=sixbit\n";
        else
            optionFile << strprintf("Name=sixbit (%s)\n", ChainTypeToString(chain));
