...
const std::string SixbitGUI::DEFAULT_UIPLATFORM =
...
SixbitGUI::SixbitGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &SixbitGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &SixbitGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &SixbitGUI::showModalOverlay);
...
void SixbitGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a sixbit address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and sixbit: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &SixbitGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &SixbitGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &SixbitGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your sixbit addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified sixbit addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file..."), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed sixbit Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard..."), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed sixbit Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a sixbit: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible sixbit command-line options").arg(CLIENT_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &SixbitGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &SixbitGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &SixbitGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &SixbitGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &SixbitGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &SixbitGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [path, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &SixbitGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &SixbitGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &SixbitGUI::createWallet);
        ...
        connect(m_migrate_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const auto& [wallet_name, info] : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, wallet_name] {
                    ...
                    connect(activity, &MigrateWalletActivity::migrated, this, &SixbitGUI::setCurrentWallet);
        ...
        connect(m_mask_values_action, &QAction::toggled, this, &SixbitGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &SixbitGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &SixbitGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &SixbitGUI::showDebugWindow);
}

void SixbitGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void SixbitGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &SixbitGUI::setCurrentWalletBySelectorIndex);
...
void SixbitGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &SixbitGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &SixbitGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &SixbitGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &SixbitGUI::showProgress);
...
#ifdef ENABLE_WALLET
void SixbitGUI::enableHistoryAction(bool privacy)
{
...
void SixbitGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &SixbitGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &SixbitGUI::removeWallet);
...
WalletController* SixbitGUI::getWalletController()
{
    return m_wallet_controller;
}

void SixbitGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &SixbitGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &SixbitGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &SixbitGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &SixbitGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &SixbitGUI::incomingTransaction);
    connect(this, &SixbitGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void SixbitGUI::removeWallet(WalletModel* walletModel)
{
...
void SixbitGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void SixbitGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void SixbitGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void SixbitGUI::setWalletActionsEnabled(bool enabled)
{
...
void SixbitGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &SixbitGUI::toggleHidden);
...
void SixbitGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void SixbitGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void SixbitGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void SixbitGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void SixbitGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void SixbitGUI::openClicked()
{
...
void SixbitGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void SixbitGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void SixbitGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void SixbitGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void SixbitGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void SixbitGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void SixbitGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void SixbitGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to sixbit network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void SixbitGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void SixbitGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void SixbitGUI::updateHeadersSyncProgressLabel()
{
...
void SixbitGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void SixbitGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &SixbitGUI::quitRequested);
...
void SixbitGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void SixbitGUI::createWallet()
{
#ifdef ENABLE_WALLET
    auto activity = new CreateWalletActivity(getWalletController(), this);
    connect(activity, &CreateWalletActivity::created, this, &SixbitGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void SixbitGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void SixbitGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &SixbitGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &SixbitGUI::show);
                e->ignore();
            }
        }
    }
...
void SixbitGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void SixbitGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void SixbitGUI::incomingTransaction(const QString& date, sixbitUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(sixbitUnits::formatWithUnit(unit, amount, true));
...
void SixbitGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void SixbitGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool SixbitGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool SixbitGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void SixbitGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void SixbitGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void SixbitGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void SixbitGUI::updateProxyIcon()
{
    std::string ip_port;
...
void SixbitGUI::updateWindowTitle()
{
...
void SixbitGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void SixbitGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void SixbitGUI::detectShutdown()
{
...
void SixbitGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void SixbitGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(SixbitGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = SixbitGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void SixbitGUI::subscribeToCoreSignals()
{
...
void SixbitGUI::unsubscribeFromCoreSignals()
{
...
bool SixbitGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<sixbitUnit> units = sixbitUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const sixbitUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, sixbitUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const sixbitUnit u : sixbitUnits::availableUnits()) {
        menu->addAction(sixbitUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(sixbitUnit newUnits)
{
    setText(sixbitUnits::longName(newUnits));
}
