...
const char *DEFAULT_GUI_PROXY_HOST = "1.0.0.1";
...
/** Call node.updateRwSetting() with sixbit 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because sixbit 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com.sixbit.sixbit/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because.sixbit 22.x will not try to read these.
...
//! Convert enabled/size values to.sixbit -prune setting.
...
//! Get pruning enabled value to show in GUI from.sixbit -prune setting.
...
//! Get pruning size value to show in GUI from.sixbit -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplaysixbitUnit")) {
        settings.setValue("DisplaysixbitUnit", QVariant::fromValue(sixbitUnit::SIXBIT));
    }
    QVariant unit = settings.value("DisplaysixbitUnit");
    if (unit.canConvert<sixbitUnit>()) {
        m_display_sixbit_unit = unit.value<sixbitUnit>();
    } else {
        m_display_sixbit_unit = sixbitUnit::SIXBIT;
        settings.setValue("DisplaysixbitUnit", QVariant::fromValue(m_display_sixbit_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if sixbit.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_sixbit_unit);
...
{
    if (new_unit.isNull() || new_unit.value<sixbitUnit>() == m_display_sixbit_unit) return;
    m_display_sixbit_unit = new_unit.value<sixbitUnit>();
    QSettings settings;
    settings.setValue("DisplaysixbitUnit", QVariant::fromValue(m_display_sixbit_unit));
    Q_EMIT displayUnitChanged(m_display_sixbit_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com.sixbit.sixbit/pull/8273
    ...
    // (https://github.com/sixbit-core/gui/issues/567).
