...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaType<sixbitUnit>("sixbitUnit");
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. sixbit_de.qm (shortcut "de" needs to be defined in.sixbit.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. sixbit_de_DE.qm (shortcut "de_DE" needs to be defined in.sixbit.qrc)
...
static const char* qt_argv = "sixbit-qt";

SixbitApplication::SixbitApplication()
...
void SixbitApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the SixbitApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", SixbitGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void SixbitApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool SixbitApplication::createOptionsModel(bool resetSettings)
{
...
void SixbitApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new SixbitGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &SixbitGUI::quitRequested, this, &SixbitApplication::requestShutdown);
...
void SixbitApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void SixbitApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool SixbitApplication::baseInitialize()
{
    return node().baseInitialize();
}

void SixbitApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &SixbitApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &SixbitApplication::handleRunawayException);
    connect(this, &SixbitApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &SixbitApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void SixbitApplication::parameterSetup()
{
...
void SixbitApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void SixbitApplication::requestInitialize()
{
...
void SixbitApplication::requestShutdown()
{
...
void SixbitApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        //.sixbit: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &SixbitGUI::handlePaymentRequest);
            connect(window, &SixbitGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void SixbitApplication::handleRunawayException(const QString &message)
{
...
void SixbitApplication::handleNonFatalException(const QString& message)
{
...
WId SixbitApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool SixbitApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", SixbitGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(sixbit);
    Q_INIT_RESOURCE(sixbit_locale);
    ...
    for (int i = 1; i < argc; i++) {
...
#ifdef ENABLE_WALLET
        if (arg.startsWith(SIXBIT_IPC_PREFIX, Qt::CaseInsensitive)) {
            invalid_token &= false;
        ...
        if (invalid_token) {
            InitError(Untranslated(strprintf("Command line contains unexpected token '%s', see sixbit-qt -h for a list of options.", argv[i])));
            QMessageBox::critical(nullptr, CLIENT_NAME,
                                  // message cannot be translated because translations have not been initialized
                                  QString::fromStdString("Command line contains unexpected token '%1', see sixbit-qt -h for a list of options.").arg(QString::fromStdString(argv[i])));
            return EXIT_FAILURE;
        }
    }
    ...
    /// 6-7. Parse sixbit.conf, determine network, switch to network specific
    ...
    //.sixbit: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
