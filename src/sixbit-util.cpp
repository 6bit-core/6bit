...
static void SetupSixbitUtilArgs(ArgsManager& argsman)
{
...
static int AppInitUtil(ArgsManager& args, int argc, char* argv[])
{
    ...
    if (HelpRequested(args) || args.GetBoolArg("-version", false)) {
        ...
        std::string strUsage = "sixbit-util utility version " + FormatFullVersion() + "\n";

        if (args.GetBoolArg("-version", false)) {
            strUsage += FormatParagraph(LicenseInfo());
        } else {
            strUsage += "\n"
                "The sixbit-util tool provides sixbit-related functionality that does not rely on the ability to access a running node. Available commands are listed below.\n"
                "\n"
                "Usage:  sixbit-util [options] [command]\n"
                "or:     sixbit-util [options] grind <hex-block-header>\n";
            strUsage += "\n" + args.GetHelpMessage();
        }
