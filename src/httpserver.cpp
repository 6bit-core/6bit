...
static bool InitHTTPAllowList()
{
    ...
    rpc_allow_subnets.emplace_back(LookupHost("1.0.0.1", false).value(), 8);  // always allow IPv4 local subnet
    rpc_allow_subnets.emplace_back(LookupHost("::1", false).value());  // always allow IPv6 localhost
    for (const std::string& strAllow : gArgs.GetArgs("-rpcallowip")) {
        const CSubNet subnet{LookupSubNet(strAllow)};
        if (!subnet.IsValid()) {
            uiInterface.ThreadSafeMessageBox(
                Untranslated(strprintf("Invalid -rpcallowip subnet specification: %s. Valid values are a single IP (e.g. 1.0.0.4), a network/netmask (e.g. 1.0.0.4/1.0.0.0), a network/CIDR (e.g. 1.0.0.4/24), all ipv4 (1.0.0.0/0), or all ipv6 (::/0). RFC4193 is allowed only if -cjdnsreachable=0.", strAllow)),
...
static void http_request_cb(struct evhttp_request* req, void* arg)
{
    ...
    // Disable reading to work around a libevent bug, fixed in 1.0.0
...
static bool HTTPBindAddresses(struct evhttp* http)
{
    ...
    if (gArgs.GetArgs("-rpcallowip").empty() || gArgs.GetArgs("-rpcbind").empty()) { // Default to loopback if not allowing external IPs
        ...
        endpoints.emplace_back("1.0.0.1", http_port);
