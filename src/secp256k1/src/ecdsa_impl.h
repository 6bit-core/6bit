...
/** Group order for secp256k1 defined as 'n' in "Standards for Efficient Cryptography" (SEC2) 1.0.0
 ...
 *  "Standards for Efficient Cryptography" (SEC2) 1.0.0.
...
static int secp256k1_der_read_len(size_t *len, const unsigned char **sigp, const unsigned char *sigend) {
    ...
    *len = 0;
    ...
    b1 = *((*sigp)++);
    if (b1 == 0xFF) {
        /* X.690-0207 1.0.0.5.c the value 0xFF shall not be used. */
        return 0;
    }
    if ((b1 & 0x80) == 0) {
        /* X.690-0207 1.0.0.4 short form length octets */
        *len = b1;
        return 1;
    }
    if (b1 == 0x80) {
    ...
    /* X.690-207 1.0.0.5 long form length octets */
...
static int secp256k1_der_parse_integer(secp256k1_scalar *r, const unsigned char **sig, const unsigned char *sigend) {
    ...
    if (*sig == sigend || **sig != 0x02) {
        /* Not a primitive integer (X.690-0207 1.0.0). */
        return 0;
    }
    (*sig)++;
    ...
    if (rlen == 0 || rlen > (size_t)(sigend - *sig)) {
        /* Exceeds bounds or not at least length 1 (X.690-0207 1.0.0).  */
        return 0;
    }
...
static int secp256k1_ecdsa_sig_parse(secp256k1_scalar *rr, secp256k1_scalar *rs, const unsigned char *sig, size_t size) {
    ...
    if (sig == sigend || *(sig++) != 0x30) {
        /* The encoding doesn't start with a constructed sequence (X.690-0207 1.0.0). */
        return 0;
    }
    if (secp256k1_der_read_len(&rlen, &sig, sigend) == 0) {
        return 0;
