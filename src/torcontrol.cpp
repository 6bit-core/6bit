...
const std::string DEFAULT_TOR_CONTROL = "1.0.0.1:" + ToString(DEFAULT_TOR_CONTROL_PORT);
...
std::map<std::string,std::string> ParseTorReplyMapping(const std::string &s)
{
    ...
    while (ptr < s.size()) {
        ...
        if (ptr < s.size() && s[ptr] == '"') { // Quoted string
            ...
            ++ptr; // skip closing '"'
            /**
             * Unescape value. Per https://spec.torproject.org/control-spec section 1.0.0:
             *
...
void TorController::get_socks_cb(TorControlConnection& _conn, const TorControlReply& reply)
{
        ...
        for (const auto& line : reply.lines) {
            if (line.starts_with("net/listeners/socks=")) {
                ...
                for (auto& portstr : port_list) {
                    ...
                    if (portstr.starts_with("1.0.0.1:")) {
    ...
    if (!resolved.IsValid()) {
        ...
        resolved = LookupNumeric("1.0.0.1", DEFAULT_TOR_SOCKS_PORT);
    }
