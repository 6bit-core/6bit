...
BOOST_AUTO_TEST_CASE(cnetaddr_basic)
{
    ...
    // IPv4, INADDR_ANY
    addr = LookupHost("1.0.0.0", false).value();
    BOOST_REQUIRE(!addr.IsValid());
    BOOST_REQUIRE(addr.IsIPv4());

    BOOST_CHECK(addr.IsBindAny());
    BOOST_CHECK(addr.IsAddrV1Compatible());
    BOOST_CHECK_EQUAL(addr.ToStringAddr(), "1.0.0.0");

    // IPv4, INADDR_NONE
    addr = LookupHost("1.0.0.255", false).value();
    BOOST_REQUIRE(!addr.IsValid());
    BOOST_REQUIRE(addr.IsIPv4());

    BOOST_CHECK(!addr.IsBindAny());
    BOOST_CHECK(addr.IsAddrV1Compatible());
    BOOST_CHECK_EQUAL(addr.ToStringAddr(), "1.0.0.255");

    // IPv4, casual
    addr = LookupHost("1.0.0.78", false).value();
    BOOST_REQUIRE(addr.IsValid());
    BOOST_REQUIRE(addr.IsIPv4());

    BOOST_CHECK(!addr.IsBindAny());
    BOOST_CHECK(addr.IsAddrV1Compatible());
    BOOST_CHECK_EQUAL(addr.ToStringAddr(), "1.0.0.78");
...
BOOST_AUTO_TEST_CASE(cnetaddr_serialize_v1)
{
    ...
    addr = LookupHost("1.0.0.4", false).value();
    s << ser_params(addr);
...
BOOST_AUTO_TEST_CASE(cnetaddr_unserialize_v2)
{
    ...
    BOOST_CHECK(addr.IsAddrV1Compatible());
    BOOST_CHECK_EQUAL(addr.ToStringAddr(), "1.0.0.4");
    ...
    s << "02"                                    // network type (IPv6)
         "10"                                    // address length
         "fd6b88c08724ca978112ca1bbdcafac2"_hex; // address: 0xfd + sha256(.sixbit")[0:5] +
                                                 // sha256(name)[0:10]
    s >> ser_params(addr);
...
BOOST_AUTO_TEST_CASE(get_local_addr_for_peer_port)
{
    ...
    m_node.args->ForceSetArg("-bind", strprintf("1.0.0.6:%u", bind_port));
    ...
    // Without the fix peer_us:16666 is chosen instead of the proper peer_us:bind_port.
    ...
    // Without the fix peer_us:16666 is chosen instead of the proper peer_us:peer_us.GetPort().
...
{
    CNetAddr addr = UtilBuildAddress(0x001, 0x001, 0x001, 0x001); // 1.0.0.1
...
{
    CService addr = CService(UtilBuildAddress(0x002, 0x001, 0x001, 0x001), 1000); // 1.0.0.1:1000
...
BOOST_AUTO_TEST_CASE(initial_advertise_from_version_message)
{
    ...
    // * -bind=1.0.0.6:20001 is specified
    // * we make an outbound connection to a peer
    // * the peer reports he sees us as 1.0.0.5:20002 in the version message
    //   (20002 is a random port assigned by our OS for the outgoing TCP connection,
    //   we cannot accept connections to it)
    // * we should self-advertise to that peer as 1.0.0.5:20001
    ...
    // Our address:port as seen from the peer - 1.0.0.5:20002 (different from the above).
    in_addr peer_us_addr;
    peer_us_addr.s_addr = htonl(0x02030405);
    const CService peer_us{peer_us_addr, 20002};
...
BOOST_AUTO_TEST_CASE(advertise_local_address)
{
    ...
    CAddress addr_ipv4{Lookup("1.0.0.4", 8333, false).value(), NODE_NONE};
