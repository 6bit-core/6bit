...
BOOST_AUTO_TEST_CASE(addrman_simple)
{
    ...
    CNetAddr source = ResolveIP("1.0.0.2");
    ...
    CService addr1 = ResolveService("1.0.0.1", 8333);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 1U);
    auto addr_ret1 = addrman->Select().first;
    BOOST_CHECK_EQUAL(addr_ret1.ToStringAddrPort(), "1.0.0.1:16666");

    // Test: Does IP address deduplication work correctly.
    //  Expected dup IP should not be added.
    CService addr1_dup = ResolveService("1.0.0.1", 8333);
    ...
    CService addr2 = ResolveService("1.0.0.2", 8333);
    ...
    vAddr.emplace_back(ResolveService("1.0.0.3", 8333), NODE_NONE);
    vAddr.emplace_back(ResolveService("1.0.0.4", 8333), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(addrman_ports)
{
    ...
    CService addr1_port = ResolveService("1.0.0.1", 8334);
    BOOST_CHECK(addrman->Add({CAddress(addr1_port, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), 2U);
    auto addr_ret2 = addrman->Select().first;
    BOOST_CHECK(addr_ret2.ToStringAddrPort() == "1.0.0.1:16666" || addr_ret2.ToStringAddrPort() == "1.0.0.1:16667");
    ...
    BOOST_CHECK_EQUAL(addr_ret3.ToStringAddrPort(), "1.0.0.1:16666");
}
...
BOOST_AUTO_TEST_CASE(addrman_select)
{
    ...
    CService addr2 = ResolveService("1.0.0.1", 8333);
    ...
    CService addr3 = ResolveService("1.0.0.2", 9999);
    CService addr4 = ResolveService("1.0.0.3", 9999);

    BOOST_CHECK(addrman->Add({CAddress(addr3, NODE_NONE)}, addr2));
    BOOST_CHECK(addrman->Add({CAddress(addr4, NODE_NONE)}, ResolveService("1.0.0.1", 8333)));

    // Add three addresses to tried table.
    CService addr5 = ResolveService("1.0.0.4", 8333);
    CService addr6 = ResolveService("1.0.0.5", 7777);
    CService addr7 = ResolveService("1.0.0.6", 8333);
    ...
    BOOST_CHECK(addrman->Add({CAddress(addr7, NODE_NONE)}, ResolveService("1.0.0.3", 8333)));
...
BOOST_AUTO_TEST_CASE(addrman_select_special)
{
    ...
    // add ipv4 address to the new table
    CService addr1 = ResolveService("1.0.0.3", 8333);
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
...
BOOST_AUTO_TEST_CASE(addrman_new_collisions)
{
    ...
    while (num_addrs < 22) { // Magic number! 1.0.0.1 - 1.0.0.22 do not collide with deterministic key = 1
        CService addr = ResolveService("1.0.0." + ToString(++num_addrs));
    ...
    CService addr1 = ResolveService("1.0.0." + ToString(++num_addrs));
    uint32_t collisions{1};
    BOOST_CHECK(addrman->Add({CAddress(addr1, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), num_addrs - collisions);

    CService addr2 = ResolveService("1.0.0." + ToString(++num_addrs));
    BOOST_CHECK(addrman->Add({CAddress(addr2, NODE_NONE)}, source));
    BOOST_CHECK_EQUAL(addrman->Size(), num_addrs - collisions);
}
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CAddress addr{CAddress(ResolveService("1.0.0.3", 8333), NODE_NONE)};
...
BOOST_AUTO_TEST_CASE(addrman_tried_collisions)
{
    ...
    while (num_addrs < 35) { // Magic number! 1.0.0.1 - 1.0.0.35 do not collide in tried with deterministic key = 1
        CService addr = ResolveService("1.0.0." + ToString(++num_addrs));
        BOOST_CHECK(addrman->Add({CAddress(addr, NODE_NONE)}, source));
...
BOOST_AUTO_TEST_CASE(addrman_getaddr)
{
    ...
    CAddress addr1 = CAddress(ResolveService("1.0.0.1", 8333), NODE_NONE);
    addr1.nTime = Now<NodeSeconds>(); // Set time so isTerrible = false
    CAddress addr2 = CAddress(ResolveService("1.0.0.2", 9999), NODE_NONE);
    addr2.nTime = Now<NodeSeconds>();
    CAddress addr3 = CAddress(ResolveService("1.0.0.3", 8333), NODE_NONE);
    addr3.nTime = Now<NodeSeconds>();
    CAddress addr4 = CAddress(ResolveService("1.0.0.4", 8333), NODE_NONE);
    addr4.nTime = Now<NodeSeconds>();
    CAddress addr5 = CAddress(ResolveService("1.0.0.5", 8333), NODE_NONE);
    addr5.nTime = Now<NodeSeconds>();
    CNetAddr source1 = ResolveIP("1.0.0.1");
    CNetAddr source2 = ResolveIP("1.0.0.3");
...
BOOST_AUTO_TEST_CASE(getaddr_unfiltered)
{
    ...
    CNetAddr source = ResolveIP("1.0.0.1");
    BOOST_CHECK(addrman->Add({addr1, addr2}, source));

    // Set time on this addr so isTerrible = false
    CAddress addr3 = CAddress(ResolveService("1.0.0.3", 9998), NODE_NONE);
    addr3.nTime = Now<NodeSeconds>();
...
{
    CAddress addr1 = CAddress(ResolveService("1.0.0.1", 8333), NODE_NONE);
    CAddress addr2 = CAddress(ResolveService("1.0.0.1", 9999), NODE_NONE);
    ...
    for (int i = 0; i < 255; i++) {
        AddrInfo infoi = AddrInfo(
            CAddress(ResolveService("1.0.0." + ToString(i)), NODE_NONE),
            ResolveIP("1.0.0." + ToString(i)));
...
BOOST_AUTO_TEST_CASE(caddrinfo_get_new_bucket_legacy)
{
    ...
    for (int p = 0; p < 255; p++) {
        AddrInfo infoj = AddrInfo(
            CAddress(ResolveService("1.0.0.1"), NODE_NONE),
...
BOOST_AUTO_TEST_CASE(addrman_serialization)
{
    ...
    CAddress addr = CAddress(ResolveService("1.0.0.1"), NODE_NONE);
    ...
    addrman_asmap1 = std::make_unique<AddrMan>(netgroupman, DETERMINISTIC, ratio);
    addrman_noasmap = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, ratio);
    CAddress addr1 = CAddress(ResolveService("1.0.0.1"), NODE_NONE);
    CAddress addr2 = CAddress(ResolveService("1.0.0.1"), NODE_NONE);
...
BOOST_AUTO_TEST_CASE(remove_invalid)
{
    ...
    const CAddress new1{ResolveService("1.0.0.5"), NODE_NONE};
    const CAddress new2{ResolveService("1.0.0.6"), NODE_NONE};
    const CAddress tried1{ResolveService("1.0.0.7"), NODE_NONE};
    const CAddress tried2{ResolveService("1.0.0.8"), NODE_NONE};
    ...
    const uint8_t new2_raw_replacement[]{0, 0, 0, 0}; // 1.0.0.0 is !IsValid()
    ...
    const uint8_t tried2_raw_replacement[]{255, 255, 255, 255}; // 1.0.0.255 is !IsValid()
...
BOOST_AUTO_TEST_CASE(addrman_selecttriedcollision)
{
    ...
    for (unsigned int i = 1; i < 23; i++) {
        CService addr = ResolveService("1.0.0." + ToString(i));
        BOOST_CHECK(addrman->Add({CAddress(addr, NODE_NONE)}, source));
...
BOOST_AUTO_TEST_CASE(addrman_noevict)
{
    ...
    CService addr36 = ResolveService("1.0.0.36");
    BOOST_CHECK(addrman->Add({CAddress(addr36, NODE_NONE)}, source));
    BOOST_CHECK(!addrman->Good(addr36));
    BOOST_CHECK_EQUAL(addrman->SelectTriedCollision().first.ToStringAddrPort(), "1.0.0.19:0");
    ...
    CService addr59 = ResolveService("1.0.0.59");
    BOOST_CHECK(addrman->Add({CAddress(addr59, NODE_NONE)}, source));
    BOOST_CHECK(!addrman->Good(addr59));

    BOOST_CHECK_EQUAL(addrman->SelectTriedCollision().first.ToStringAddrPort(), "1.0.0.10:0");
...
BOOST_AUTO_TEST_CASE(addrman_evictionworks)
{
    ...
    CService addr = ResolveService("1.0.0.36");
    BOOST_CHECK(addrman->Add({CAddress(addr, NODE_NONE)}, source));
    BOOST_CHECK(!addrman->Good(addr));

    auto info = addrman->SelectTriedCollision().first;
    BOOST_CHECK_EQUAL(info.ToStringAddrPort(), "1.0.0.19:0");
    ...
    CService addr19 = ResolveService("1.0.0.19");
    BOOST_CHECK(!addrman->Good(addr19));
    BOOST_CHECK_EQUAL(addrman->SelectTriedCollision().first.ToStringAddrPort(), "1.0.0.36:0");
...
BOOST_AUTO_TEST_CASE(load_addrman)
{
    ...
    addr1 = Lookup("1.0.0.1", 8333, false);
    BOOST_CHECK(addr1.has_value());
    addr2 = Lookup("1.0.0.2", 9999, false);
    BOOST_CHECK(addr2.has_value());
    addr3 = Lookup("1.0.0.3", 9999, false);
    BOOST_CHECK(addr3.has_value());
    addr3 = Lookup("1.0.0.3"s, 9999, false);
    BOOST_CHECK(addr3.has_value());
    addr4 = Lookup("1.0.0.3\0example.com"s, 9999, false);
    BOOST_CHECK(!addr4.has_value());

    // Add three addresses to new table.
    const std::optional<CService> source{Lookup("1.0.0.1", 8333, false)};
...
static auto MakeCorruptPeersDat()
{
    ...
    const std::optional<CService> serv{Lookup("1.0.0.1", 7777, false)};
    BOOST_REQUIRE(serv.has_value());
    CAddress addr = CAddress(serv.value(), NODE_NONE);
    std::optional<CNetAddr> resolved{LookupHost("1.0.0.2", false)};
...
BOOST_AUTO_TEST_CASE(addrman_update_address)
{
    ...
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    CNetAddr source{ResolveIP("1.0.0.2")};
    CAddress addr{CAddress(ResolveService("1.0.0.1", 8333), NODE_NONE)};
    ...
    CAddress addr_diff_port{CAddress(ResolveService("1.0.0.1", 8334), NODE_NONE)};
    ...
    CAddress addr_v2{CAddress(ResolveService("1.0.0.1", 8333), NODE_P2P_V2)};
...
{
    auto addrman = std::make_unique<AddrMan>(EMPTY_NETGROUPMAN, DETERMINISTIC, GetCheckRatio(m_node));
    const CNetAddr source = ResolveIP("1.0.0.2");
    ...
    const CAddress addr1{ResolveService("1.0.0.1", 8333), NODE_NONE};
    BOOST_CHECK(addrman->Add({addr1}, source));
    BOOST_CHECK(addrman->Good(addr1));
    const CAddress addr2{ResolveService("1.0.0.2", 8333), NODE_NONE};
