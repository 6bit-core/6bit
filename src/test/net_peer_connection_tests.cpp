...
BOOST_FIXTURE_TEST_CASE(test_addnode_getaddednodeinfo_and_connection_detection, PeerTest)
{
    ...
    {
        ASSERT_DEBUG_LOG("Added connection to 1.0.0.1:16666 peer=1");
        AddPeer(id, nodes, *peerman, *connman, ConnectionType::MANUAL, /*onion_peer=*/false, /*address=*/"1.0.0.1");
    ...
    for (int i = 0; i < 10; ++i) {
        ASSERT_DEBUG_LOG("Not opening a connection to localhost, already connected to 1.0.0.1:16666");
    ...
    BOOST_CHECK(!connman->AddNode({/*m_added_node=*/"1.0.0.1", /*m_use_v2transport=*/true}));
