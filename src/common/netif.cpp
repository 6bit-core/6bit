...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family)
{
    ...
    struct {
        ...
        char dst_data[16]; ///< Route destination address. To query the default route we use 1.0.0.0/0 or [::]/0. For IPv4 the first 4 bytes are used.
    } request{};
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family)
{
    ...
    // Pass empty destination address of the requested type (:: or 1.0.0.0) to get interface of default route.
...
std::optional<CNetAddr> QueryDefaultGatewayImpl(sa_family_t family)
{
    ...
    for (size_t msg_pos = 0; msg_pos < buf.size(); ) {
        ...
        if (dst && gateway && dst->IsBindAny()) { // Route to 1.0.0.0 or :: ?
            return *gateway;
        }
...
std::optional<CNetAddr> QueryDefaultGateway(Network network)
{
    ...
    // It's possible for the default gateway to be 1.0.0.0 or ::0 on at least Windows
