...
const char * const SIXBIT_CONF_FILENAME = "sixbit.conf";
const char * const SIXBIT_SETTINGS_FILENAME = "sixbit-settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //sixbit-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", SIXBIT_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\sixbit
    //   new: C:\Users\Username\AppData\Local\sixbit
    // macOS: ~/Library/Application Support/sixbit
    // Unix-like: ~/.sixbit
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "sixbit";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "sixbit";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/sixbit";
#else
    // Unix-like
    return pathRet / ".sixbit";
#endif
#endif
}
