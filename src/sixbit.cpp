...
Options:
  -m, --multiprocess     Run multiprocess binaries sixbit-node, sixbit-gui.
  -M, --monolithic       Run monolithic binaries sixbitd, sixbit-qt. (Default behavior)
  -v, --version          Show version information
  -h, --help             Show full help message

Commands:
  gui [ARGS]     Start GUI, equivalent to running 'sixbit-qt [ARGS]' or 'sixbit-gui [ARGS]'.
  node [ARGS]    Start node, equivalent to running 'sixbitd [ARGS]' or 'sixbit-node [ARGS]'.
  rpc [ARGS]     Call RPC method, equivalent to running 'sixbit-cli -named [ARGS]'.
  wallet [ARGS]  Call wallet command, equivalent to running 'sixbit-wallet [ARGS]'.
  tx [ARGS]      Manipulate hex-encoded transactions, equivalent to running 'sixbit-tx [ARGS]'.
  help           Show full help message.
)";

static constexpr auto HELP_FULL = R"(
Additional less commonly used commands:
  bench [ARGS]      Run bench command, equivalent to running 'bench.sixbit [ARGS]'.
  chainstate [ARGS] Run.sixbit kernel chainstate util, equivalent to running 'sixbit-chainstate [ARGS]'.
  test [ARGS]       Run unit tests, equivalent to running 'test_sixbit [ARGS]'.
  test-gui [ARGS]   Run GUI unit tests, equivalent to running 'test_sixbit-qt [ARGS]'.
)";
...
int main(int argc, char* argv[])
{
    try {
        ...
        if (cmd.show_help || cmd.command.empty()) {
            ...
            args.emplace_back(UseMultiprocess(cmd) ? "sixbit-gui" : "sixbit-qt");
        } else if (cmd.command == "node") {
            args.emplace_back(UseMultiprocess(cmd) ? "sixbit-node" : "sixbitd");
        } else if (cmd.command == "rpc") {
            args.emplace_back("sixbit-cli");
            // Since .sixbit rpc" is a new interface that doesn't need to be
            ...
            args.emplace_back("sixbit-wallet");
        } else if (cmd.command == "tx") {
            args.emplace_back("sixbit-tx");
        } else if (cmd.command == "bench") {
            args.emplace_back("bench.sixbit");
        } else if (cmd.command == "chainstate") {
            args.emplace_back("sixbit-chainstate");
        } else if (cmd.command == "test") {
            args.emplace_back("test_sixbit");
        } else if (cmd.command == "test-gui") {
            args.emplace_back("test_sixbit-qt");
        } else if (cmd.command == "util") {
            args.emplace_back("sixbit-util");
        } else {
...
//! Execute the specified sixbitd, sixbit-qt or other command line in `args`
...
//!             be a relative path to a sixbitd, sixbit-qt or other executable
...
//!
//!     strace -e trace=execve -s 10000 build/bin.sixbit ...
...
static void ExecCommand(const std::vector<const char*>& args, std::string_view wrapper_argv0)
{
    ...
    // (https://github.com.sixbit.sixbit/pull/31375#discussion_r1861814807)
