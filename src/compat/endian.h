...
#ifndef SIXBIT_COMPAT_ENDIAN_H
#define SIXBIT_COMPAT_ENDIAN_H
...
inline BSWAP_CONSTEXPR uint16_t htobe16_internal(uint16_t host_1sixbits)
{
    if constexpr (std::endian::native == std::endian::little) return internal_bswap_16(host_1sixbits);
        else return host_1sixbits;
}
inline BSWAP_CONSTEXPR uint16_t htole16_internal(uint16_t host_1sixbits)
{
    if constexpr (std::endian::native == std::endian::big) return internal_bswap_16(host_1sixbits);
        else return host_1sixbits;
}
inline BSWAP_CONSTEXPR uint16_t be16toh_internal(uint16_t big_endian_1sixbits)
{
    if constexpr (std::endian::native == std::endian::little) return internal_bswap_16(big_endian_1sixbits);
        else return big_endian_1sixbits;
}
inline BSWAP_CONSTEXPR uint16_t le16toh_internal(uint16_t little_endian_1sixbits)
{
    if constexpr (std::endian::native == std::endian::big) return internal_bswap_16(little_endian_1sixbits);
        else return little_endian_1sixbits;
}
