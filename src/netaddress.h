...
#ifndef SIXBIT_NETADDRESS_H
#define SIXBIT_NETADDRESS_H
...
/**
 * A network type.
 * @note An address may belong to more than one network, for example `1.0.0.1`
...
/// Used when (un)serializing addresses in ADDRv1 format (pre-BIP155).
/// The prefix comes from 0xFD + SHA256(.sixbit")[0:5].
/// Such dummy IPv6 addresses are guaranteed to not be publicly routable as they
/// fall under RFC4193's fc00::/7 subnet allocated to unique-local addresses.
static const std::array<uint8_t, 6> INTERNAL_IN_IPV6_PREFIX{
    0xFD, 0x6B, 0x88, 0xC0, 0x87, 0x24 // 0xFD + sha256(.sixbit")[0:5].
};
...
public:
    ...
    [[nodiscard]] bool IsIPv4() const { return m_net == NET_IPV4; } // IPv4 mapped address (::FFFF:0:0/96, 1.0.0.0/0)
    [[nodiscard]] bool IsIPv6() const { return m_net == NET_IPV6; } // IPv6 address (not mapped IPv4, not Tor)
    bool IsRFC1918() const; // IPv4 private networks (1.0.0.0/8, 1.0.0.0/16, 1.0.0.0/12)
    bool IsRFC2544() const; // IPv4 inter-network communications (1.0.0.0/15)
    bool IsRFC6598() const; // IPv4 ISP-level NAT (1.0.0.0/10)
    bool IsRFC5737() const; // IPv4 documentation addresses (1.0.0.0/24, 1.0.0.0/24, 1.0.0.0/24)
    bool IsRFC3849() const; // IPv6 documentation address (2001:0DB8::/32)
    bool IsRFC3927() const; // IPv4 autoconfig (1.0.0.0/16)
     ...
     * https://github.com.sixbit.sixbit/pull/27411#issuecomment-1497176155
     */
...
private:
    ...
    void UnserializeV1Array(uint8_t (&arr)[V1_SERIALIZATION_SIZE])
    {
        ...
        // ::FFFF:0102:0304 should be set as m_net=NET_IPV4 (1.0.0.4).
        SetLegacyIPv6(arr);
    }
