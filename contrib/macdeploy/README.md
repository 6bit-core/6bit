...
When complete, it will have produced `Sixbit Core.zip`.
...
An Apple ID and enabled cookies are required to download this file.
...
apt install cpio
git clone https://github.com/6bit-core/6bit/apple-sdk-tools.git
...
All builds must target an Apple SDK version compatible with the release. These SDKs are free to download, but not redistributable.
...
- Builders use Guix to create an unsigned release. This outputs an unsigned ZIP which
  users may choose to self-codesign, and run. It also outputs an unsigned app structure
  in the form of a tarball.
- The Apple keyholder uses this unsigned app to create a detached signature, using the
  included script. Detached signatures are available from this [repository](https://github.com/sixbit-core/sixbit-detached-sigs).
