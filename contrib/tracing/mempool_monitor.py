...
try:
    ...
    print("Error: bcc not installed (sudo apt install bpfcc-tools python3-bpfcc)")
    sys.exit(1)

""" Example logging Sixbit Core mempool events using the mempool:added,
...
def main(pid):
    print(f"Hooking into sixbitd (PID {pid})")
    sixbitd_with_usdts = USDT(pid=int(pid))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    sixbitd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    sixbitd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    sixbitd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    sixbitd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[sixbitd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "<pid of sixbitd>")
        exit(1)
