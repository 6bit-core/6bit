...
"""Example logging Sixbit Core utxo set cache flushes utilizing
    the utxocache:flush tracepoint."""

# USAGE: ./contrib/tracing/log_utxocache_flush.py <pid of sixbitd>
...
def main(pid):
    print(f"Hooking into sixbitd with pid {pid}")
    sixbitd_with_usdts = USDT(pid=int(pid))

    sixbitd_with_usdts.enable_probe(
        probe="utxocache:flush", fn_name="trace_flush")
    b = BPF(text=program, usdt_contexts=[sixbitd_with_usdts])
