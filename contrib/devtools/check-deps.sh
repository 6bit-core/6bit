...
declare -A LIBS
LIBS[cli]="libsixbit_cli.a"
LIBS[common]="libsixbit_common.a"
LIBS[consensus]="libsixbit_consensus.a"
LIBS[crypto]="libsixbit_crypto.a"
LIBS[node]="libsixbit_node.a"
LIBS[util]="libsixbit_util.a"
LIBS[wallet]="libsixbit_wallet.a"
LIBS[kernel]="libsixbit_kernel.a"
...
for tool in "${NM:-nm}" c++filt comm awk sed; do
...
extract_symbols() {
    ...
    for lib in "${!LIBS[@]}"; do
        for lib_path in ${LIBS[$lib]}; do
            ${NM:-nm} -g -o "$lib_path" 2>/dev/null | { grep ' [TW] ' || true; } | awk '{print $3, $1}' > "${temp_dir}/${lib}_exports.txt"
            ${NM:-nm} -g -o "$lib_path" 2>/dev/null | { grep ' U ' || true; } | awk '{print $3, $1}' > "${temp_dir}/${lib}_imports.txt"
...
obj_names() {
    local symbol="$1"
    local txt_file="$2"
    local symbol_escaped
    symbol_escaped=$(printf '%s\n' "$symbol" | sed 's/[][\/.^$*]/\\&/g')

    sed -n "s/^$symbol_escaped [^:]*:\([^:]*\):.*$/\1/p" "$txt_file" | sort -u
}
...
mapfile -t TARGETS < <(lib_targets)
cmake --build "$BUILD_DIR" -j"$JOBS" -t "${TARGETS[@]}"

TEMP_DIR="$(mktemp -d)"
LIBDIR="$BUILD_DIR/lib"
[ -d "$LIBDIR" ] || LIBDIR="$BUILD_DIR"
cd "$LIBDIR"
