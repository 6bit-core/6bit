...
Provides utilities for managing copyright headers of `The Sixbit Core
...
```
// Copyright (c) <firstYear>-<lastYear> The Sixbit Core developers
```
will be updated to:
```
```
...
Generates a sixbit.conf file in `share/examples/` by parsing the output from `sixbitd --help`. This script is run during the
release process to include a sixbit.conf with the release binaries and can also be run by users to generate a file locally.
...
```bash
BUILDDIR=$PWD/my-build-dir contrib/devtools/gen-sixbit-conf.sh
```
