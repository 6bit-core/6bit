...
be used in Sixbit Core with the `-asmap` option.
...
to binary before being used with Sixbit Core.
...
- present in the first but not the second (`# 1.0.0.0/24 was AS9723`),
- or present in the second but not the first (`# was unassigned`).

```
1.0.0.0/19 AS26496 # was AS20738
# 1.0.0.0/24 was AS9723
1.0.0.0/23 AS400080 # was unassigned
...
the `sixbit-cli getnodeaddresses` command.
...
```
sixbit-cli getnodeaddresses 0 > addrs.json
```
