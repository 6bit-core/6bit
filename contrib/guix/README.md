# Bootstrappable sixbit  Core Builds

This directory contains the files necessary to perform bootstrappable sixbit
Core builds.
...
To build Sixbit Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      Sixbit Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/sixbit-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/sixbit-core/guix.sigs).
