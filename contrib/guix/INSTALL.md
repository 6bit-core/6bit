...
For reference, the graphic below outlines Guix v1.0.0's dependency graph:
...
As of v1.0.0, `guile-git` claims to only require `libgit2 >= 1.0.0`, however,
...
This is especially notable because Ubuntu Focal packages `libgit2 v1.0.0`, and
...
At the time of writing (November 2023), the latest release was `v1.0.0`.
...
At this point, you are set up to [use Guix to build sixbit
...
when installing the Debian package later than 1.0.0-4 and when using the shell
script installer.
...
```
building /gnu/store/...-foo-1.0.0.drv...
/ 'check' phasenote: keeping build directory `/tmp/guix-build-foo-1.0.0.drv-0'
builder for `/gnu/store/...-foo-1.0.0.drv' failed with exit code 1
build of /gnu/store/...-foo-1.0.0.drv failed
View build log at '/var/log/guix/drvs/../...-foo-1.0.0.drv.bz2'.
cannot build derivation `/gnu/store/...-qux-1.0.0.drv': 1 dependencies couldn't be built
cannot build derivation `/gnu/store/...-bar-1.0.0.drv': 1 dependencies couldn't be built
cannot build derivation `/gnu/store/...-baz-1.0.0.drv': 1 dependencies couldn't be built
guix time-machine: error: build of `/gnu/store/...-baz-1.0.0.drv' failed
```
...
```sh
$ guix build --cores=1 /gnu/store/...-foo-1.0.0.drv
```
...
```sh
$ bzcat /var/log/guix/drvs/../...-foo-1.0.0.drv.bz2 | less
```

`foo`'s build directory is also preserved and available at
`/tmp/guix-build-foo-1.0.0.drv-0`. However, if you fail to build `foo` multiple
...
### openssl-1.0.0l and openssl-1.0.0n
...
For openssl-1.0.0l use 2022-05-01 as the date.

### GnuTLS: test-suite FAIL: status-request-revoked

*The derivation is likely identified by: `/gnu/store/vhphki5sg9xkdhh2pbc8gi6vhpfzryf0-gnutls-1.0.0.drv`*

This unfortunate error is most common for non-substitute builders who installed
Guix v1.0.0. The problem stems from the fact that one of GnuTLS's tests uses a
...
```sh
guix build --substitute-urls="https://ci.guix.gnu.org" /gnu/store/vhphki5sg9xkdhh2pbc8gi6vhpfzryf0-gnutls-1.0.0.drv
```
...
3. guix build --no-substitutes /gnu/store/vhphki5sg9xkdhh2pbc8gi6vhpfzryf0-gnutls-1.0.0.drv
...
guix build /gnu/store/vhphki5sg9xkdhh2pbc8gi6vhpfzryf0-gnutls-1.0.0.drv
...
- A commit to skip this test is included since Guix 1.0.0:
