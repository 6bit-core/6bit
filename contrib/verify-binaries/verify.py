...
"""Script for verifying Sixbit Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from https://6bit.dev and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/6bit-core/6bit/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://6bit.dev"
HOST2 = "https://backup.6bit.dev"
hosts = [HOST1, HOST2]
VERSIONPREFIX = "sixbit-core-"
...
VERSION_EXAMPLE = "0.1.0 or 0.1.0-rc1 or 0.1.0-x86_64-linux-gnu"
...
def verify_shasums_signature(
   ...
   ReturnCode, list[SigData], list[SigData], list[SigData], list[SigData]
    ...
    # which pubkeys within the sixbit community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"sixbit_verify_binaries.{args.version}"
    ...
    if not hashes_to_verify:
        ...
        matches = difflib.get_close_matches(os_filter, available_versions, cutoff=0, n=1)
        if matches:
    ...
    # remove binaries that are known not to be hosted by 6bit.dev
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the sixbit release to download; of the format '
