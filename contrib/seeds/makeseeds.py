...
#
# Generate seeds.txt from sixbit DNS sources
#
...
PATTERN_AGENT = re.compile(r"^/sixbit:")
...
def parse_args():
    argparser = argparse.ArgumentParser(description='Generate a list of sixbit node seed ip addresses.')
...
def main():
    ...
    # Relax filtering for early network.
    # ips = [ip for ip in ips if PATTERN_AGENT.match(ip['agent'])]
    print(f'{ip_stats(ips):s} Require a known and recent user agent', file=sys.stderr)
    # Sort by availability (and use last success as tie breaker)
    ips.sort(key=lambda x: (x['uptime'], x['lastsuccess'], x['ip']), reverse=True)
    # Filter out hosts with multiple sixbit ports, these are likely abusive
    ips = filtermultiport(ips)
    print(f'{ip_stats(ips):s} Filter out hosts with multiple sixbit ports', file=sys.stderr)
