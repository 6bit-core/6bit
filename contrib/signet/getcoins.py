...
DEFAULT_GLOBAL_FAUCET = 'https://faucet.6bit.dev/claim'
DEFAULT_GLOBAL_CAPTCHA = 'https://faucet.6bit.dev/captcha'
...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing sixbit-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='sixbit-cli', help='sixbit-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='sixbit address to which the faucet should send')
...
parser.add_argument('sixbit_cli_args', nargs='*', help='Arguments to pass on to sixbit-cli (default: -signet)')

args = parser.parse_args()

if args.sixbit_cli_args == []:
    args.sixbit_cli_args = ['-signet']


def sixbit_cli(rpc_command_and_params):
    argv = [args.cmd] + args.sixbit_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = sixbit_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = sixbit_cli(['getnewaddress', 'faucet', 'bech32'])
